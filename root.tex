...
\begin{figure*}
\centering
{\includegraphics[width=\linewidth]{./figs/plot2.pdf}}
...
\begin{figure}
\centering
{\includegraphics[width=\linewidth]{./figs/histOnly.pdf}}
\caption{Resource utilization (BRAM) with growing problem space}
\label{fig:barplot}
\end{figure}

Along with minimum error rate validation, we also validate the scalability of Meta-DNN in terms of hardware resource utilization. The bar plot in fig. \ref{fig:barplot} illustrates that Meta-DNN manages to keep the BRAM requirements from growing exponentially. The $100\%$ utilization of BRAM indicates  invalid allocation of memory resource. With that given, the baseline did not passed the synthesis design after the problem space grew beyond $2^{26}$. The sparsity for each state space instance was kept at maximum while registering BRAM resource information. For example, for $2^{30}$, we used $45\%$ sparsity for Meta-DNN. It is interesting to note that Meta-DNN experienced synthesis failure for $2^{30}$ dimension. However, even after failing the synthesis design, it did not exhausted the BRAM resource entirely. That is mainly because of the LUT-based modular structure of Meta-circuit (see fig \ref{fig:iteration}), as it decentralizes the memory load per timestep due to layer multiplexing design. 
